...
select 
    ...
    ,product_name 
from product
where created_at in (SELECT date_sub(CURRENT_DATE, INTERVAL 30 day))
;
...
select 
	customer_id
    ,concat(first_name, ' ', last_name) as name
    ,created_at from customer
where created_at in (select date_sub(CURRENT_DATE, INTERVAL 1 month))
;
...
select 
	month(a.order_date) as order_month 
    ,month(b.payment_date) as payment_month
	,sum(a.total_amount) as order_total
    ,sum(b.amount_paid) as payment_total 
from orders a 
inner join payment b using(order_id)
group by month(a.order_date), month(b.payment_date)
order by 1,2
;
...
select 
	store_name, 
    sum(cast(b.quantity_in_stock as SIGNED)) as stock 
from store a
inner join inventory b using(store_id)
group by store_name
order by stock desc
